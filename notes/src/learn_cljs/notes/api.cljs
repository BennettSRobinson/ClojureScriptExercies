...
(defn create-note! [note]
  (do-request! :post "/notes" note
               (with-error-handling  #(emit! :notes/created %))))

(defn get-notes! []
  (do-request! :get "/notes"
               (with-error-handling #(emit! :notes/recieved %))))
...
(defn get-note! [id]
  (do-request! :get (str "/notes/" id)
               (with-error-handling #(emit! :note/recieved %))))

(defn get-tags! []
  (do-request! :get "/tags"
               (with-error-handling #(emit! :tags/recieved %))))
...
(defn tag-note! [note-id tag-id]
  (do-request! :put (str "/notes/" note-id "/tags/" tag-id)
               (with-error-handling #(emit! :note/tagged {:note-id note-id
                                                          :tag-id tag-id}))))